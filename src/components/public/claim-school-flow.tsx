...
export function ClaimSchoolFlow({ directorySchools, currentUser }: ClaimSchoolFlowProps) {
  ...
  const isAlreadyLinked = Boolean(currentUser && currentUser.schoolId);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 sm:space-y-8">
      ...
      {isAlreadyLinked ? (
        <div className="space-y-6 rounded-2xl border border-background-secondary bg-white p-5 sm:p-8 text-center shadow-sm">
          ...
          <div className="space-y-3">
            <h2 className="text-2xl font-bold text-navy-900">Already Linked to a School</h2>
            <p className="mx-auto max-w-xl text-sm leading-relaxed text-slate-600">
              Your account is already linked to <strong className="text-navy-900">{currentUser?.schoolName || 'a school'}</strong>.
              {currentUser?.schoolStatus ? (
                <> School Status: <strong className="text-navy-900 uppercase">{currentUser.schoolStatus}</strong>.</>
              ) : null}
            </p>
            <p className="mx-auto max-w-xl text-xs text-slate-500">
              School claims are blocked for accounts that are already linked to an existing school profile.
            </p>
          </div>
          <div className="pt-4">
